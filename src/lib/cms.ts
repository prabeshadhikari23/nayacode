...
const initializeDefaultContent = () => {
  ...
  if (!localStorage.getItem(CMS_SERVICES_KEY)) {
    localStorage.setItem(CMS_SERVICES_KEY, JSON.stringify(defaultServices));
  }
  if (!localStorage.getItem(CMS_PORTFOLIO_KEY)) {
    localStorage.setItem(CMS_PORTFOLIO_KEY, JSON.stringify(defaultPortfolio));
  }
  if (!localStorage.getItem(CMS_PARTNERS_KEY)) {
    localStorage.setItem(CMS_PARTNERS_KEY, JSON.stringify(defaultPartners));
  }
  if (!localStorage.getItem(CMS_CONTACT_KEY)) {
    localStorage.setItem(CMS_CONTACT_KEY, JSON.stringify(defaultContact));
  }
...
export const updateCMSContent = (key: string, value: any): void => {
...
export const getCMSContentByKey = (key: string): string => {
  const content = getCMSContent();
...
export const updateServices = (services: ServiceItem[]): void => {
...
export const updateContactInfo = (contact: ContactInfo): void => {
...
export const addFormSubmission = (submission: Omit<FormSubmission, 'id' | 'submittedAt' | 'status'>): void => {
...
export const updateFormSubmission = (id: string, updates: Partial<FormSubmission>): void => {
  const submissions = getFormSubmissions();
